...
function tryRegisterContextLostEvents(): void {
  try {
    createAPICanvas();

    // Register to contextLost Events
    globalAPICanvas.addEventListener("contextlost", this.handleCanvasContextLost);
    globalAPICanvas.addEventListener(
      "contextrestored",
      this.handleCanvasContextRestored
    );
    console.log("Registered Context Lost Events");
  } catch (e) {
    console.log(
      `Exception during CreateTestableCanvasElement. Exception: ${e}.`
    );
    globalAPICanvas = null;
  }
}

function handleCanvasContextLost(domEvent): void {
  console.log("HandleCanvasContextLost");
}

function handleCanvasContextRestored(domEvent): void {
